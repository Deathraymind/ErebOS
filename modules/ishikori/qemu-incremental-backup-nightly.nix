...
with lib; let
  ...
  backupScript = pkgs.writeShellScriptBin "qemu-incremental-backup" ''
    ...
    # Generic path (was /tmp/pelican-daily.lock, a legacy per-VM name).
    exec 9>/run/qemu-incremental-backup.lock
    ...
    for VM in "''${VMS[@]}"; do
      ...
      # VMs hosted on a peer are a clean skip, NOT an error.
      ...
      # ---- PEER VALIDATION PASS (was GUARD 3) -----------------------------
      # Build TARGETS: the peers we will actually replicate to tonight.
      # Runs BEFORE any local commit/snapshot so a genuine split brain still
      # leaves the disk chain untouched. Three independent per-peer verdicts:
      #
      #   * VM running on a REACHABLE peer -> genuine split brain (two live
      #       copies). HARD ABORT this VM and touch nothing locally. This is
      #       the one fatal condition -- replicating over a live base would
      #       roll the peer back under a running guest.
      #   * peer UNREACHABLE -> soft skip (warn + mark partial), NOT fatal.
      #       A peer we can't reach is a peer we can't send to, so we also
      #       can't roll it back. Skipping it is safe; the OTHER peers and
      #       the local snapshot still happen. (This is the whole point of
      #       the rewrite: one flaky node can't sabotage the nightly run.)
      #   * parent dataset MISSING on peer -> soft skip (warn + mark partial).
      #       An unprovisioned/misconfigured target; syncoid would only fail.
      #       Keeps a node with no matching vmpool (e.g. a compute-only box
      #       accidentally left in peerIps) from erroring every single night.
      TARGETS=()
      for PEER in "''${PEERS[@]}"; do
        if ! peer_vms=$(ssh -o ConnectTimeout=10 -o BatchMode=yes "root@$PEER" \
              "virsh list --state-running --name" 2>&1); then
          echo "WARN: peer $PEER unreachable; skipping it for $VM (not fatal): $peer_vms"
          OVERALL_STATUS=1
          continue
        fi

        if grep -qx "$VM" <<< "$peer_vms"; then
          echo "ABORT: $VM also running on $PEER (split brain). Refusing to touch $VM tonight."
          OVERALL_STATUS=1
          continue 2   # bail the whole VM, before any local commit/snapshot
        fi

        # Peer is up and not running the VM -- confirm it can actually receive.
        if ! ssh -o ConnectTimeout=10 -o BatchMode=yes "root@$PEER" \
              "zfs list -H -o name '$PARENT'" >/dev/null 2>&1; then
          echo "WARN: peer $PEER has no '$PARENT' dataset; skipping it for $VM (not fatal)."
          OVERALL_STATUS=1
          continue
        fi
      ...
      # Commit local data (always worth doing -- the local ZFS snapshot below
      # is a valid rollback point even if TARGETS ended up empty).
      if [ "$active" = "$BASE" ]; then
      ...
      # ---- Stale overlay guard --------------------------------------------
      # The re-freeze below creates $IMGDIR/$VM.daily fresh. If that file
      # already exists AND isn't the live disk, it's an orphan: a prior
      # cycle's overlay that rode in with the syncoid dataset and got stranded
      # when a live migration landed the guest on the flat base. snapshot-
      # create-as won't overwrite it, so it wedges this VM every night and
      # fails the whole unit. We re-query the live disk so the guest's own
      # view is authoritative: any overlay that isn't the live disk is
      # provably not in the running chain, and qemu doesn't hold it open.
      #   mv, not rm: if a migration ever *didn't* flatten cleanly, that
      #     overlay is the only copy of those writes -- verify, then delete
      #     the .orphan.* files by hand.
      #   NOT --reuse-external: the stale overlay's backing file and contents
      #     are from the old chain; reusing it would roll the guest back onto
      #     dead data.
      OVERLAY_FILE="$IMGDIR/$VM.$OVERLAY_NAME"
      live=$(virsh domblklist "$VM" | awk -v d="$DISK" '$1==d {print $2}')
      if [ -e "$OVERLAY_FILE" ] && [ "$OVERLAY_FILE" != "$live" ]; then
        STASH="$OVERLAY_FILE.orphan.$(date +%s)"
        echo "WARNING: stale overlay $OVERLAY_FILE present; live disk is $live. Stashing to $STASH."
        if ! mv "$OVERLAY_FILE" "$STASH"; then
          echo "ERROR: could not stash stale overlay for $VM. Skipping."
          OVERALL_STATUS=1
          continue
        fi
      fi
      ...
      # MULTI-NODE SYNC: incrementally replicate the dataset to every VALID
      # target (reachable, not running the VM, has $PARENT). syncoid creates
      # its own sync snapshot, finds the newest common snapshot with the peer,
      # rolls the peer back to it (recv -F) if it diverged (e.g. the VM ran
      # there last week), and sends only changed blocks (--no-stream: direct
      # incremental, no intermediate snapshot replay). Every prior snapshot
      # is a rollback point.
      if [ ''${#TARGETS[@]} -eq 0 ]; then
        echo "No reachable/valid replication targets for $VM; took a local snapshot only."
      else
        for PEER in "''${TARGETS[@]}"; do
          echo "Replicating $DATASET to $PEER..."
          if ! syncoid --identifier=nightly --compress=${cfg.syncoidCompress} --no-stream \
...
in {
  options.services.qemu-incremental-backup-nightly = {
    ...
    peerIps = mkOption {
      ...
      default = [];
      description = ''
        Target IP addresses of the peer nodes to replicate to. Do NOT include
        this node's own IP. For an N-node full mesh, list every OTHER mesh node.
        A peer must (a) be reachable over passwordless root SSH and (b) have the
        datasetParent below present under the same name -- peers that fail either
        check are skipped for the night (warned, non-fatal), not aborted on.
      '';
      example = ["192.168.1.99" "192.168.1.101"];
    };

    datasetParent = mkOption {
      type = types.str;
      default = "vmpool/images";
      description = "Parent ZFS dataset containing one child dataset per VM (e.g. vmpool/images/caddy). Must exist under the same name on all peers listed in peerIps.";
    };

    syncoidCompress = mkOption {
      type = types.str;
      default = "none";
      description = ''
        syncoid --compress value. "none" is optimal for the 10GbE node1<->node2
        direct link (CPU, not bandwidth, is the bottleneck there). For a mesh
        peer reached only over 1GbE LAN, "zstd-fast" or "lz4" can help.
      '';
      example = "zstd-fast";
    };
